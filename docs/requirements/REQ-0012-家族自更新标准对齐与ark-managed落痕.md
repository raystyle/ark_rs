...
status: implemented
priority: must
trace: cargo test --release --locked 全绿 225 项（semver 门/判型/自替换回滚/锁收割/落痕五件新测）；门禁全绿；对线 review 见 diary
---
...
| --- | --- | --- | --- |
| 1 | stable 段双通道优先：镜像 stable 滚动段下载腿优先，任一步失败整对回落 GitHub | stable 通道沿用 D51 镜像元数据读序（边车锚）加官方 API 兜底；镜像命中而资产网络性失败补官方链（对线 F2） | **部分差异**：stable 通道改家族形：判新与元数据走官方 latest API 一次（tag 加 digest，browse 同构「判新走 GitHub tag，镜像段资产名不带版本无法反查」在 ark 同理），下载腿镜像 stable 段优先（官方 digest 锚）、网络性失败整对回落官方 URL。dev 通道（默认）维持 D51 镜像元数据优先（用户裁定 2026-09-18 两连发在先，dev 滚动源无版本语义），家族契约「dev 加 stable 双通道是否随仓开放由仓裁」 |
| 2 | digest 锚硬校验，哈希不符硬拒不回落 | 镜像段锚不符视同镜像失败回落官方（D44 工具面红线沿用）；官方腿终态 digest 硬校验已拒（不符即败） | **差异收口**：selfupdate 两通道镜像腿改「官方/边车 digest 锚 + 不符硬拒 Err」；网络性失败（连接错、404 未命中）仍整对回落官方（与 browse「任一步失败整对回落」并列条款一致：网络性回落、校验性硬拒）。install 工具面维持 D44「有锚必校验、不符回落」（pin sha 是官方真源锚，语义不同，入档分家） |
