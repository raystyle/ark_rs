...
| `issue new` | filed, id, url, tool, version, endpoint（REQ-0009；list 另出 count 与 `#id` 概览行，count 是本次返回条数非在册总数，默认 limit 100 即服务端上限，恰打满出 stderr 截断提示，`--before <id>` keyset 游标翻更早一页（家族统一标准 #52/#53）；show 出单条全字段加 url） |
