...
- `env_var` — 读环境变量：空白值视同未设，返回 None（旧名读回已随 ome 关键字剔除批撤除，2026-09-18）。
- `exe_suffix` — 可执行文件后缀。
...
- `legacy_alias_target` — `ome` 别名落点（D41 C 过渡载体已停建，2026-09-14 全舰队 ome 水位清零收口；
...
- `remove_legacy_alias` — 清理 `ome` 别名副本（幂等：不在位返回 false 静默；best-effort：失败返回 Err 由调用方告警，
...
- `user_env_write_blocked` — 用户面写入总闸门（测试隔离）：`ARK_TEST_NO_PATH_REG=1` 时，**所有**用户环境写入面
