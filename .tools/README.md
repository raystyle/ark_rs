...
| `seed.py` | 软件资产与 ark 产物对 env.ohmygh.com（R2）种子同步：域面 diff（`--plan` 只读无凭据）与 rclone 上传（路线 A 加 B，D27；catalog 三件套归 ohmycloud catalog-seed，B 承接 R015 五）；D41 B 起自产段配 ark-* 主名（`--ark-dev`/`--ark-stable`/`--ark-published`）；ome/ 段与 ome-* 兼容写已撤（全舰队 ome 水位清零，2026-09-14 收口）；skip 面护栏：部分 skip 落 WARN 列名、零到件红灯拦（三段报数；ARK_SEED_ALLOW_SKIP=1 豁免旧 tag 窗口期重灌，仅 --ark-stable/--ark-dev 面；2026-09-17 补审 F2）；REQ-0007 批一：`--ark-published` 双段同灌（版本段 copy immutable 加 stable sync 排除保 D46 msvc 窗一窗）加双段逐名核对齐备红灯，段制 sync 清旧（rclone 语义实证：排除即保护、不带 --delete-excluded）；REQ-0008 窗口：包形名族到则灌（裸件终版窗跳过即 phase 无关） | `uv run --script .tools/seed.py [--plan\|--ark-dev --tag dev\|--ark-stable --tag <v*>\|--ark-published --tag <v*>]` |
...
| `release.ps1` | 本地编译打包发布命令面（REQ-0007 批二，build-release 标准三段式第一二段）：tag 锚闸加工作树洁净闸加发布预检三件（gh 登录、远端 tag 锚本 sha、无既有 release）加版本一致性闸加测试闸（cargo test 加 md 四件套加 aidoc 漂移）加 linux 本职与 win-gnu 交叉本地构建加 lan-mac 实机构建（rsync 源码树）加逐件 .sha256 边车加跨实机冒烟（逐字对）加 gh draft 传齐后 edit --draft=false --latest 直发；第三段自动播种归 r2-seed workflow（published 事件）；`-IncludePackages` 加挂包形（REQ-0008 双挂窗：单顶层目录 ark-<净triple> 直放二进制加 README 加 LICENSE，win zip 他 tar.gz，包形冒烟含拼名与 sha 等价断言） | `pwsh -NoProfile -File .tools/release.ps1 -Tag <v*> [-DryRun] [-SkipTagCheck] [-SkipMac] [-IncludePackages]` |
