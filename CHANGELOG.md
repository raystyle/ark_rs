...
- **包形窗口一版**（REQ-0008，总台合流令三点全准后动工）：`ark self update` 资产读序扩四层（包形主名 ark-<净triple>.zip/.tar.gz 先、gnu 裸件回退臂次、D46 msvc 与 ome 兼容层照旧）；包形判新机制不动（归档 sha 锚校验下载后解包取内层二进制，等值判定在二进制间进行）；`release.ps1 -IncludePackages` 开关形双挂（缺省裸件单形 = 2.2.0 终版形；包形构建加拼名与 sha 等价与双平台直跑冒烟）；seed 面包形到则灌（phase 无关）；LICENSE 入仓与 AGENTS 双通道口径句（差10）。
